...
export const R013B_EAGER_WITHOUT_CASCADE: Rule = {
  id: 'R013B-EAGER-WITHOUT-CASCADE',
  description:
    "Détecte les relations `fetch: 'eager'` sans `onDelete` explicite — orphelins populés silencieusement.",
  ...
  apply(ctx: RuleContext): Finding[] {
    ...
    for (const schema of ctx.schemas) {
      ...
      for (const [relName, rel] of Object.entries(relations)) {
        ...
        const suggestedOnDelete =
          rel.type === 'one-to-many' ? 'cascade' : (rel.required ? 'cascade' : 'set-null')

        findings.push({
          ruleId: R013B_EAGER_WITHOUT_CASCADE.id,
          severity: R013B_EAGER_WITHOUT_CASCADE.defaultSeverity,
          message:
            `Relation \`${schema.name}.${relName}\` (${rel.type} → ${rel.target}) est eager ` +
