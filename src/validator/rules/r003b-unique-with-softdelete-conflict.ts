...
export const R003B_UNIQUE_WITH_SOFTDELETE_CONFLICT: Rule = {
  id: 'R003B-UNIQUE-WITH-SOFTDELETE-CONFLICT',
  description:
    'Détecte les index unique non-sparse sur des schémas avec soft-delete — bloque les réinsertions après soft-delete.',
  ...
  apply(ctx: RuleContext): Finding[] {
    ...
    for (const schema of ctx.schemas) {
      ...
      if (!hasNativeSoftDelete && !manualPattern) continue

      // 2. Pour chaque index unique non-sparse, flag.
      for (const idx of schema.indexes ?? []) {
        if (!idx.unique) continue
        if (idx.sparse === true) continue   // déjà partial → OK

        const indexFieldNames = Object.keys(idx.fields ?? {})
        if (indexFieldNames.length === 0) continue
        ...
        const allFieldsAreSdFields = indexFieldNames.every(
          f => f === sdTimestamp || f === sdFlag,
        )
        if (allFieldsAreSdFields) continue
        ...
        findings.push({
          ...
          message:
            `Index unique \`${indexLabel}\` sur \`${schema.name}\` (soft-delete actif) sans \`sparse: true\` — ` +
