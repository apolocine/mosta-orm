...
export const R016_AUDIT_EMAIL_AS_STRING: Rule = {
  ...
  apply(ctx: RuleContext): Finding[] {
    ...
    const hasUserSchema = ctx.schemas.some(s =>
      s.name === 'User' || s.name === 'RbacUser' || s.name === 'AuthUser',
    )
    if (!hasUserSchema) return findings   // pas de User → suggestion pas pertinente

    for (const schema of ctx.schemas) {
      for (const [fieldName, fieldDef] of Object.entries(schema.fields ?? {})) {
        if (!auditFields.has(fieldName)) continue
