...
import type { Finding, Rule, RuleContext, SoftDeletePattern } from '../types.js'

export const R003_SOFT_DELETE: Rule = {
  ...
  apply(ctx: RuleContext): Finding[] {
    ...
    const schemaPatterns = new Map<string, { pattern: SoftDeletePattern; nativeSoftDelete: boolean }>()
    ...
    for (const schema of ctx.schemas) {
      ...
      const found = patterns.find(p =>
        p.flag in fields && p.timestamp in fields,
      )
      if (found) {
    ...
    if (schemaPatterns.size === 0) return []   // aucun soft-delete partout, OK
    ...
    if (uniqueManualPatterns.size > 1) {
      ...
      for (const p of manualPatterns) counts.set(p, (counts.get(p) ?? 0) + 1)
      const majority = [...counts.entries()].sort((a, b) => b[1] - a[1])[0]?.[0]

      for (const [schemaName, info] of schemaPatterns) {
        if (info.nativeSoftDelete) continue
        if (info.pattern.flag === majority) continue
        findings.push({
...
function buildUnifySuggestion(schemaName: string, current: SoftDeletePattern, target: string): string {
  return [
...
function buildMigrateToNativeSuggestion(schemaName: string): string {
  return [
