...
# Test P0-2: M2M delete junction cleanup — Oracle XE
...
# Oracle XE — user devuser / Devapass@26
ORACLE_URI="${ORACLE_URI:-oracle://devuser:devpass26@localhost:1521/XEPDB1}"

echo -e "\n${CYAN}════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}  Test P0-2 : M2M Delete Junction Cleanup — Oracle XE${NC}"
echo -e "${CYAN}════════════════════════════════════════════════════════${NC}\n"

cd "$ORM_DIR"
RESULT=$(DIALECT=oracle DB_URI="$ORACLE_URI" node tests-scripts/test-m2m-delete.mjs 2>/dev/null)

if [ -z "$RESULT" ]; then
  echo -e "  ${RED}✗ Script failed. Errors:${NC}"
  DIALECT=oracle DB_URI="$ORACLE_URI" node tests-scripts/test-m2m-delete.mjs 2>&1 | head -30
  exit 1
fi
