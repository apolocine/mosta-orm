...
DB_FILE=$(mktemp /tmp/test-m2m-delete-XXXXXX.db)
trap "rm -f $DB_FILE" EXIT

echo -e "\n${CYAN}════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}  Test P0-2 : M2M Delete Junction Cleanup — SQLite${NC}"
echo -e "${CYAN}════════════════════════════════════════════════════════${NC}\n"

cd "$ORM_DIR"
RESULT=$(DIALECT=sqlite DB_URI="$DB_FILE" node tests-scripts/test-m2m-delete.mjs 2>/dev/null)

if [ -z "$RESULT" ]; then
  echo -e "  ${RED}✗ Script failed. Errors:${NC}"
  DIALECT=sqlite DB_URI="$DB_FILE" node tests-scripts/test-m2m-delete.mjs 2>&1 | head -30
  exit 1
fi
