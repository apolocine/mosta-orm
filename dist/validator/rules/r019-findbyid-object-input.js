...
export const R019_FINDBYID_OBJECT_INPUT = {
    id: 'R019-FINDBYID-OBJECT-INPUT',
    description: "Détecte les `findById(entity.relationField)` où relationField est une relation ORM — passe un objet sous fetch:'eager', source d'ambiguïté.",
    defaultSeverity: 'warning',
    needsSource: true,
    apply(ctx) {
        const findings = [];
        if (!ctx.sourceFiles || ctx.sourceFiles.length === 0)
            return findings;
        ...
        for (const sf of ctx.sourceFiles) {
            ...
            for (const call of calls) {
                ...
                if (propAccess.getName() !== 'findById')
                    continue;
                ...
                if (!relationNames.has(accessedField))
                    continue;
                ...
                const replacement = fullCallText.replace(argAccess.getText(), `extractRelId(${argAccess.getText()})`);
                ...
                findings.push({
                    ...
                    message: `findById(\`${argAccess.getText()}\`) — \`${accessedField}\` est une relation ORM, ` +
