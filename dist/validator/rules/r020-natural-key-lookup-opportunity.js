...
export const R020_NATURAL_KEY_LOOKUP_OPPORTUNITY = {
    id: 'R020-NATURAL-KEY-LOOKUP-OPPORTUNITY',
    description: 'Repère les `findOne({ uniqueField: x })` qui pourraient aussi s\'écrire `findById({ uniqueField: x })` — opportunité, pas obligation.',
        ...
        for (const sf of ctx.sourceFiles) {
            ...
            for (const call of calls) {
                ...
                const match = uniqueShapes.find(shape => {
                    if (shape.fieldSet.size !== propSet.size)
                        return false;
                ...
                findings.push({
                    ...
                    message: `findOne(${filterText}) correspond à l'index unique \`${match.fieldNames.join('+')}\` ` +
