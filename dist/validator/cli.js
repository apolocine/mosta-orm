...
for (let i = 1; i < args.length; i++) {
    ...
    switch (a) {
        ...
        case '--out':
            ...
            break;
        case '--ignore':
            ignore.push(...args[++i].split(','));
            break;
        case '--ci':
            ci = true;
            break;
        case '--max-warnings':
            maxWarnings = Number(args[++i]);
            break;
        case '--verbose':
            verbose = true;
            break;
...
async function main() {
    ...
    switch (format) {
        case 'json':
            output = formatJson(report, true);
            break;
        case 'markdown':
            output = formatMarkdown(report);
            break;
        case 'text':
            output = formatText(report, { verbose });
            break;
    }
    if (outFile) {
...
async function loadSchemasFromDir(dir) {
    ...
    async function walk(d) {
        ...
        for (const e of entries) {
            if (e === 'node_modules' || e === 'dist' || e === '.next')
                continue;
            const full = join(d, e);
            ...
            if (!/\.(ts|tsx|js|mjs)$/.test(full))
                continue;
            if (full.endsWith('.d.ts'))
                continue;
            try {
                const mod = await jiti.import(full);
                ...
                if (process.env.MOSTAJS_ORM_VALIDATOR_DEBUG) {
                    console.error(`[debug] failed to load ${full}: ${e.message}`);
                }
            }
        }
    }
}
function isEntitySchema(x) {
