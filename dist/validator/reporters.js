...
function formatLocation(f) {
    ...
    if (f.location.schema)
        parts.push(f.location.schema);
    if (f.location.field)
        parts.push(`.${f.location.field}`);
    if (f.location.file)
        parts.push(`(${f.location.file}${f.location.line ? `:${f.location.line}` : ''})`);
    return parts.join('');
}
function c(s, color, enabled) {
...
// ─── JSON (CI / diff) ────────────────────────────────────────────────
export function formatJson(report, pretty = true) {
    return pretty
