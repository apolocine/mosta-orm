...
import { DEFAULT_SOFT_DELETE_PATTERNS, DEFAULT_AUDIT_BY_FIELDS, DEFAULT_THRESHOLDS, } from './types.js';
...
import { R007_REDUNDANT_DERIVED_FIELD, R008_BEST_EFFORT_RESOLVER, R011_LEGACY_DEAD_CODE, R012_DUPLICATE_IMPLEMENTATION, R018_EXTERNAL_SCHEMA_OVERSCOPED, } from './rules/r007-r008-r011-r012-r018-stubs.js';
...
export async function validateSchemas(schemas, options = {}) {
    const t0 = Date.now();
    const opts = {
    ...
    const ctx = { schemas, options: opts, sourceFiles };
    const findings = [];
    for (const rule of DEFAULT_RULES) {
        if (options.ignore?.includes(rule.id))
            continue;
        if (rule.needsSource && !sourceFiles)
            continue;
        try {
            ...
            findings.push({
                ruleId: rule.id,
                severity: 'error',
                message: `Rule '${rule.id}' a lancé une exception : ${e.message}`,
                location: {},
    ...
    const countBySeverity = { error: 0, warning: 0, info: 0, hint: 0 };
    const countByRule = {};
...
function readSourceTree(sourceRoot) {
