 ...
 */
export function diffSchemas(oldSchemas, newSchemas) {
    const ops = [];
    ...
    for (const [name, newSchema] of newMap) {
        ...
        // Timestamps
        if (!oldSchema.timestamps && newSchema.timestamps)
            ops.push({ type: 'addTimestamps', entity: name });
        if (oldSchema.timestamps && !newSchema.timestamps)
            ops.push({ type: 'removeTimestamps', entity: name });
        // Soft-delete
        if (!oldSchema.softDelete && newSchema.softDelete)
            ops.push({ type: 'addSoftDelete', entity: name });
        if (oldSchema.softDelete && !newSchema.softDelete)
            ops.push({ type: 'removeSoftDelete', entity: name });
        // Discriminator
 ...
 */
export function generateMigrationSQL(ops) {
    const statements = [];
    for (const op of ops) {
        switch (op.type) {
            case 'addEntity':
