...
export class OrmIntrospectionError extends Error {
    ...
    input;
    availableFields;
    constructor(message, schemaName, input, availableFields) {
        super(message);
 ...
 */
export function findMatchingUniqueIndex(schema, obj) {
 ...
 */
export function resolveLookup(schema, idOrEntity) {
    ...
    if (typeof idOrEntity === 'object') {
        ...
        throw new OrmIntrospectionError(`Cannot resolve findById on '${schema.name}': object lacks 'id' and no unique index matches. ` +
            `Object fields: [${objKeys}]. ` +
            `Available unique indexes: [${availableUniqueFields || 'none'}].`, schema.name, idOrEntity, Object.keys(obj));
    }
    // Type primitif inattendu (boolean, symbol, function…)
    throw new OrmIntrospectionError(`Cannot resolve findById on '${schema.name}': unsupported input type '${typeof idOrEntity}'.`, schema.name, idOrEntity, []);
}
/**
 ...
 */
export function extractRelId(value) {
